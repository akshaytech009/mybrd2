...
<script type = 'text/javascript'>
	...
	function chkfrm(){
		...
		var code = document.modifyform.code.value;
		var name = document.modifyform.name.value;
		var address1 = document.modifyform.address1.value;
		var pincode = document.modifyform.pincode.value;
		var email = document.modifyform.email.value;
		var pcp = document.modifyform.pcp.value;
...
<div class ="container">
	...
	<form name = "modifyform" action="CustController" method="post" onsubmit ="return chkfrm();"><br>
  	
  	<div class="form-group">
    	<label for="exampleInputEmail1">Customer Code</label>
    	<input type="text" class="form-control" id="exampleInputEmail1" name="code" value=${cust.code} readonly>
  	</div>
  	<div class="form-group">
    	<label for="exampleInputEmail1">Name</label>
    	<input type="text" class="form-control" id="exampleInputEmail1" name="name" value=${cust.name}>
  	</div>
  	<div class="form-group">
  		<label for="comment">Address1</label>
  		<input class="form-control" rows="5" id="comment" name="address1" value=${cust.address1} >
	</div>
  	<div class="form-group">
  		<label for="comment">Address2</label>
  		<input class="form-control" rows="5" id="comment" name="address2" value=${cust.address2}>
	</div>
  	<div class="form-group">
    	<label for="exampleInputEmail1">Pin Code</label>
    	<input type="text" class="form-control" id="exampleInputEmail1" name="pincode" value=${cust.pincode}>
  	</div>
  	<div class="form-group">
    	<label for="exampleInputEmail1">Email</label>
    	<input type="text" class="form-control" id="exampleInputEmail1" name="email" value=${cust.email}>
  	</div>
  	<div class="form-group">
    	<label for="exampleInputEmail1">Contact</label>
  		<input type="text" class="form-control" id="exampleInputEmail1" name="contact" value=${cust.contact}>
  	</div>
  	<div class="form-group">
    	<label for="exampleInputEmail1">Primary Contact Person</label>
    	<input type="text" class="form-control" id="exampleInputEmail1" name="pcp" value=${cust.pcp}>
  	</div>
  	<select name="dropdown" style="color: #000000;">
  			<option value="A">Active</option>
  			<option value="I">Inactive</option>
		</select><br><br>	
		<input name = "button1" type="submit" class="btn btn-success" value = "Update2"/>
